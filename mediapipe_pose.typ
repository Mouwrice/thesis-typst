...
The model takes an image as input with a fixed size of 256 by 256 pixels and 3 values for each pixel providing RGB values, @mediapipe-network. The model uses "a regression approach that is supervised by a combined heat map/offset prediction of all keypoints" @blazepose, @cnn-pose. The left-hand side outputs the heat maps and offset maps. Both the centre and left-hand side of the network are trained using the provided heatmap and offset loss. Afterwards the heatmaps and output maps output layers are removed and the regression encoder on the right-hand side is trained. This shows that there is no use in increasing the resolution in order to get an increase in the prediction accuracy. The measurements also confirm this hypothesis.
